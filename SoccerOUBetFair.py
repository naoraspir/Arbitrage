...
def GetFinalDf(trading, event_id_lst, BetType):
    ...
    m_types=trading.betting.list_market_types(market_catalogue_filter)
    m_types.sort(key=lambda x: x.market_type)
    for type in m_types:
        if "OVER_UNDER" in type.market_type and "CARDS" not in type.market_type and "CORNR" not in type.market_type and "TEAM" not in type.market_type:
            bet_types.append(type.market_type)
    for bet_type in bet_types:
        market_catalogue_filter = betfairlightweight.filters.market_filter(event_ids=event_id_lst,
                                                                           market_type_codes=[bet_type])
        marketProjection = ['EVENT', 'RUNNER_DESCRIPTION', 'COMPETITION']
        # recive market catalog with all relevant information.
        market_c=trading.betting.list_market_catalogue(
            filter=market_catalogue_filter,
            max_results='1000',
            market_projection=marketProjection
        )
        for market in market_c:
            market_catalogues.append(market)
    ...
    for market_cat_object in market_catalogues:
        ...
        runners_lst.append(market_cat_object.runners[0].runner_name.upper().split(" G")[0])
        selection_ids.append(market_cat_object.runners[0].selection_id)
        runners_lst.append(market_cat_object.runners[1].runner_name.upper().split(" G")[0])
...
if __name__=="__main__":
    final_df_betfair, trading=GetBetFairDF()